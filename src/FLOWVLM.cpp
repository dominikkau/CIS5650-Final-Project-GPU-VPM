...
void FLOWVLM::solve(Wing& wing,
    std::function<std::vector<double>(const std::vector<double>&, double)> Vinf,
    double t,
    std::function<Eigen::Vector3d(const std::vector<double>&, double)> vortexsheet,
    ...
    // Obtain horseshoes
    std::vector<Horseshoe> HSs = getHorseshoes(wing, t, extraVinf, extraVinfArgs);
    ...
    // Calls the solver
	Eigen::VectorXd Gammas = s.solve(HSs, Vinfs, t, vortexsheet);
...
std::vector<Horseshoe> FLOWVLM::getHorseshoes(Wing& wing, double t = 0.0,
    std::function<std::vector<double>(int, double)> extraVinf = {},
    const std::vector<double>& extraVinfArgs = {}) {
...
std::vector<double> FLOWVLM::Vind(Wing& wing, const std::vector<double>& X, double t = 0.0,
    ...
    for (int i = 1; i <= wing.get_m(); ++i) {
		...
		VLMSolver s;
        std::vector<double> inducedVelocity = s.V(HS, X, ign_col, ign_infvortex, only_infvortex);
...
std::string FLOWVLM::get_hash(const std::string& var) {
	VLMSolver s;
    auto it = s.HS_hash.find(var);
    if (it != s.HS_hash.end()) {
